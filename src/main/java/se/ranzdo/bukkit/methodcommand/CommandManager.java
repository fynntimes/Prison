...
public class CommandManager implements CommandExecutor {
    ...
    private Map<String, PluginCommand> rootPCommands = new HashMap<>();
    private Map<Class<?>, ArgumentHandler<?>> argumentHandlers =
        new HashMap<Class<?>, ArgumentHandler<?>>();
    private Map<org.bukkit.command.Command, RootCommand> rootCommands =
        new HashMap<org.bukkit.command.Command, RootCommand>();
    ...
    public void registerCommands(Object commands) {
        for (Method method : commands.getClass().getDeclaredMethods()) {
            ...
            PluginCommand rootPcommand = rootPCommands.get(identifiers[0]);

            if (rootPcommand == null) {
                rootPCommands.put(identifiers[0], registerCommand(identifiers[0]));
                rootPcommand = rootPCommands.get(identifiers[0]);
            }
    ...
    private PluginCommand registerCommand(String... aliases) {
    ...
    private PluginCommand getCommand(String name, Plugin plugin) {
        ...
        } catch (SecurityException | IllegalArgumentException | InstantiationException | InvocationTargetException | NoSuchMethodException | IllegalAccessException e) {
            e.printStackTrace();
        }
